...
class QualityAuditor(Agent):
    ...
    async def execute(self, ctx: AgentContext) -> AgentResult:
        ...
        for index, satellite in enumerate(satellites, start=1):
            ...
            satellite["enrichment_confidence"] = _enrichment_confidence(
                satellite, completeness
            )
            ...
            if index % 1000 == 0:
                ctx.emit.progress(
                    self.name,
                    min(0.95, index / max(1, len(satellites))),
                    f"Audited {index}/{len(satellites)} satellites",
                )
        ...
        operator_and_purpose = sum(
            ...
            if row.get("operator_name") not in {None, ""}
            and row.get("purpose_primary") not in {None, ""}
        )
        mass_coverage = sum(
            1 for row in updated_rows if row.get("dry_mass_kg") not in {None, ""}
        )
        ...
        manifest = {
            ...
            "active_records": sum(
                1 for row in updated_rows if row.get("active_status") == "active"
            ),
            "research_candidates": int(
                ctx.state.get("_run_summary", {}).get("research_candidates", 0)
            ),
            "researched_records": int(
                ctx.state.get("_run_summary", {}).get("researched_records", 0)
            ),
            "accepted_llm_records": int(
                ctx.state.get("_run_summary", {}).get("accepted_llm_records", 0)
            ),
            "anomaly_count": len(findings),
            "source_breakdown_json": json.dumps(
                dict(source_breakdown), separators=(",", ":")
            ),
            ...
            "space_track_mode": str(
                ctx.state.get("_run_summary", {}).get(
                    "space_track_mode", "prefer_cache"
                )
            ),
            "field_completion_rates_json": json.dumps(
                coverage_json, separators=(",", ":")
            ),
        ...
        summary = {
            ...
            "cached_sources": ctx.state.get("_run_summary", {}).get(
                "cached_sources", []
            ),
            "stale_sources": ctx.state.get("_run_summary", {}).get("stale_sources", []),
            "space_track_mode": manifest["space_track_mode"],
            "llm_research_status": ctx.state.get("_run_summary", {}).get(
                "llm_research_status"
            ),
            "coverage_operator_purpose_pct": round(
                (operator_and_purpose / max(1, len(updated_rows))) * 100, 2
            ),
            "coverage_mass_pct": round(
                (mass_coverage / max(1, len(updated_rows))) * 100, 2
            ),
        ...
        return AgentResult(
            records_processed=len(updated_rows),
...
def _validate_satellite(
    run_id: str, satellite: dict[str, object]
) -> list[dict[str, object]]:
    findings: list[dict[str, object]] = []
    if satellite.get("inclination_deg") is not None and not (
        0 <= float(satellite["inclination_deg"]) <= 180
    ):
        findings.append(
            _finding(
                run_id,
                ...
                "warning",
                "invalid_inclination",
                "Inclination is outside the valid 0-180 range",
            )
        )
    if satellite.get("eccentricity") is not None and not (
        0 <= float(satellite["eccentricity"]) < 1
    ):
        findings.append(
            _finding(
                run_id,
                ...
                "Eccentricity is outside the valid 0-1 range",
            )
        )
    if (
        satellite.get("perigee_km") is not None
        and satellite.get("apogee_km") is not None
    ):
        if float(satellite["perigee_km"]) > float(satellite["apogee_km"]):
            findings.append(
                _finding(
                    ...
                    satellite["norad_id"],
                    "warning",
                    "invalid_orbit_bounds",
                    "Perigee exceeds apogee",
                )
            )
        ...
        if value and not _is_reasonable_date(str(value)):
            findings.append(
                _finding(
                    ...
                    satellite["norad_id"],
                    "warning",
                    f"invalid_{date_field}",
                    f"{date_field} is not a plausible ISO date",
                )
            )
    for code_field in (
        ...
        if value and not str(value).isalpha():
            findings.append(
                _finding(
                    ...
                    satellite["norad_id"],
                    "warning",
                    f"invalid_{code_field}",
                    f"{code_field} should be alphabetic",
                )
            )
    return findings
...
def _field_coverage(
    run_id: str, rows: list[dict[str, object]]
    ...
    for field_name in COMPLETENESS_FIELDS:
        populated = sum(
            ...
            and not (
                ...
                and not str(row.get(field_name)).strip()
            )
        )
...
def _finding(
    run_id: str, norad_id: object | None, severity: str, code: str, message: str
) -> dict[str, object]:
    return {
        "run_id": run_id,
        "norad_id": norad_id,
